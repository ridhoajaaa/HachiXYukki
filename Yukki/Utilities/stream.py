...
async def start_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        final_output = await CallbackQuery.message.reply_text(
                f"<b>💡 Track added to queue</b> » <code>{position}</code>\n\n🏷️ <b>Name: </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n⏱️ <b>Duration:</b> <code>{duration_min}</code> \n🎧 <b>Request By: </b>{CallbackQuery.from_user.mention}",
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_text(
                f"<b>💡 Track added to queue</b> » <code>{position}</code>\n\n🏷️ <b>Audio: </b> [Given Audio Via Telegram]({link})\n⏱️ <b>Duration:</b> <code>{duration_min}</code> \n🎧 <b>Request By: </b>{message.from_user.mention}",
        ...
        await mystic.delete()
        cap = f"<b>🏷️ Name: </b>[Given Audio Via Telegram]({link})\n🎧 <b>Request By:</b> {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Audio.jpeg",
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
        )
        await start_timer(
