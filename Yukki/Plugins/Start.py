        ...
        try:
            ...
            if member.id in OWNER_ID:
                return await message.reply_text(
                    f"{MUSIC_BOT_NAME}'s Owner [{member.mention}] Coming Stay Alert."
                )
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"A member of {MUSIC_BOT_NAME}'s Sudo User[{member.mention}] waw."
                )
            return
        except:
            return
...
async def useradd(_, message: Message):
    ...
    await asyncio.gather(
        message.delete(),
        message.reply_text(
            f"Thanks To Adding Me In {message.chat.title}.\n{MUSIC_BOT_NAME} Alive.",
            reply_markup=InlineKeyboardMarkup(out[1]),
        ),
    )
...
async def okaybhai(_, CallbackQuery):
    ...
    await CallbackQuery.edit_message_text(
        text=f"Thanks To Adding Me In {CallbackQuery.message.chat.title}.\n{MUSIC_BOT_NAME} Alive.",
        reply_markup=InlineKeyboardMarkup(out[1]),
    )
...
async def EVE(_, CallbackQuery):
    ...
    if not is_non_admin:
        ...
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\nAdmin Command Mode to **Everyone**\n\nNow anyone in this group can skip, pause, resume, stop music.\n\nChange Completed By @{checking}",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
        await CallbackQuery.answer(
            "Command For Everyone", show_alert=True
        )
...
async def AMS(_, CallbackQuery):
    ...
    if not is_non_admin:
        await CallbackQuery.answer(
            "Commands Set Untuk ADMINS SAJA", show_alert=True
        )
    else:
        await CallbackQuery.answer("Changes Saved")
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "USERLIST":
        ...
        if not _playlist:
            ...
            j = 0
            await CallbackQuery.edit_message_text(
                "Fetching Authorised Users... Sabar Ngentot"
            )
