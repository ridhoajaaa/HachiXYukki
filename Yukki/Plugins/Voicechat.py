...
async def activevc(_, message: Message):
    ...
    if await is_active_chat(message.chat.id):
        mystic = await message.reply_text("Sabar Ngentot... Getting Queue..")
        dur_left = db_mem[message.chat.id]["left"]
        duration_min = db_mem[message.chat.id]["total"]
        got_queue = get_queue.get(message.chat.id)
        if not got_queue:
            await mystic.edit(f"*Nothing music in queue, Request again babi*")
        fetched = []
        ...
        await message.reply_text(f"*Nothing music in queue, Request again babi*")
