...
@app.on_message(
    filters.command("play", prefixes="!", prefixes2="/") & filters.group
)
@checker
@logging
@PermissionCheck
...
async def play(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            "Your Is __Anonymous__ In This Group!\nGo back to personal account."
        )
    audio = (
    ...
    if audio:
        mystic = await message.reply_text(
            "🔄 Memproses Audio... Sabar Ngentot!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
                return await mystic.edit(
                    "Live Streaming Terputar...Stop Untuk Play music"
                )
            else:
                pass
        except:
        ...
        if audio.file_size > 1073741824:
            return await mystic.edit_text(
                "Size Nya Kelebihan Ngentot"
            )
        duration_min = seconds_to_min(audio.duration)
        duration_sec = audio.duration
        if (audio.duration) > DURATION_LIMIT:
            return await mystic.edit_text(
                f"**Duration Limit Exceeded**\n\n**Izin Duration: **{DURATION_LIMIT_MIN} minute(s)\n**Batas Duration:** {duration_min} minute(s)"
            )
        file_name = (
        ...
        mystic = await message.reply_text("🔄 Memproses URL... Sabar Ngentot!")
        ...
        return await message.reply_photo(
            photo=thumb,
            caption=f"**🏷 Name:** [{title[:25]}](https://www.youtube.com/watch?v={videoid})\n\n⏱️ **Duration:** `{duration_min} Mins`\n💡 **Info:** [Get More Information](https://t.me/{BOT_USERNAME}?start=info_{videoid})",
        ...
        if len(message.command) < 2:
            ...
            response = await message.reply_text(
                    f"**Usage:** /play /pl /p [Name Music Atau Link Youtube Atau Reply Ke Audio]\n\n**Example:** /pl desahan janda.",
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return
...
async def Music_Stream(_, CallbackQuery):
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Live Streaming Playing...Stop To Play Music",
                show_alert=True,
            )
        else:
            pass
    ...
    if str(duration) == "None":
        ...
        return await CallbackQuery.edit_message_text(
            "**Live Stream Detected**\n\nWant to Play live stream? This will stop the currently playing music (if any) and will start the live video stream.",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Its Not For You", show_alert=True
        )
    ...
    await CallbackQuery.answer(f"Preccses:- {title[:20]}", show_alert=True)
