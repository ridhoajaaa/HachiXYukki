...
async def mhelp_command(_, message):
...
👀 **Vies:** `{views}`
⏰ **release time:** {published}
🎥 **Channel Name:** {channel}
🏷 **Channel Link:** [Click This]({channellink})
...
async def shikhar(_, CallbackQuery):
    text, keyboard = await mhelp_parser(CallbackQuery.from_user.mention)
...
async def mhelp_button(client, query):
