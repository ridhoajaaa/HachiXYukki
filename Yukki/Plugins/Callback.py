...
async def forceclose(_, CallbackQuery):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Your Tidak Di IzinKan Melakukan Ini.", show_alert=True
        )
...
async def admin_risghts(_, CallbackQuery):
    ...
    if not await is_active_chat(CallbackQuery.message.chat.id):
        return await CallbackQuery.answer(
            "Nothing plays in voice chat.", show_alert=True
        )
    chat_id = CallbackQuery.message.chat.id
    if command == "pausecb":
        if not await is_music_playing(chat_id):
            return await CallbackQuery.answer(
                "Music Already Paused", show_alert=True
            )
        await music_off(chat_id)
        await pause_stream(chat_id)
        await CallbackQuery.message.reply_text(
            f"<b>⏸ Music playback pause by</b> {CallbackQuery.from_user.mention}!\n\n"
    ...
    if command == "resumecb":
        ...
        await CallbackQuery.message.reply_text(
            f"<b>▶️ Music playback resume by</b> {CallbackQuery.from_user.mention}!\n\n"
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            await CallbackQuery.message.delete()
            await CallbackQuery.answer(
                "Skip. No Music In Queue", show_alert=True
            )
            return
        else:
            ...
            if str(finxx) == "raw":
                await CallbackQuery.message.delete()
                await CallbackQuery.answer("Skip!", show_alert=True)
            ...
            if str(finxx) == "s1s":
                ...
                mystic = await CallbackQuery.message.reply_text(
                    "**Skip! Please be patient to replace the video....**"
                )
                ...
                await CallbackQuery.message.delete()
                await CallbackQuery.answer(
                    "Skip! Playlist Playing....", show_alert=True
                )
                mystic = await CallbackQuery.message.reply_text(
                ...
                status = "SKIP TRACK"
                thumb = await gen_thumb(videoid, status, CallbackQuery.from_user.id,)
                buttons = primary_markup(
                    videoid,
                    CallbackQuery.from_user.id,
                    duration_min,
...
async def play_playlist(_, CallbackQuery):
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Live Streaming Terputar...Stop Untuk Play Playlist",
                show_alert=True,
            )
        else:
            pass
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"User Ini Tidak Memiliki Playlist DI Server.", show_alert=True
        )
    else:
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Play Playlist {third_name}.\n\nRequest By:- {CallbackQuery.from_user.first_name}"
        )
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                cap = f"<b>Started Streaming</b> \n\n🏷️ <b>Info:</b> [Get More Information](https://t.me/{BOT_USERNAME}?start=info_{videoid}) \n🎧 <b>Request By:</b> {CallbackQuery.from_user.mention}"
                final_output = await CallbackQuery.message.reply_photo(
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=cap,
        ...
        if for_p == 1:
            ...
            if await isPreviewUp(preview):
                await CallbackQuery.message.reply_photo(
                    photo=preview,
                    caption=f"Ini Queued Playlist Untuk {third_name}.\n\nPlayed by :- {CallbackQuery.from_user.mention}",
                    quote=False,
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
                await m.delete()
            ...
            await CallbackQuery.message.reply_text(
                "Hanya 1 Music Di Playlist.. Tidak ada lagi musik untuk ditambahkan ke queue."
            )
        if for_t == 1:
            await start_timer(
                send_video,
...
async def group_playlist(_, CallbackQuery):
    ...
    if not _count:
        sex = await CallbackQuery.message.reply_text(
            f"Welcome To {MUSIC_BOT_NAME}'s Playlist Featurs.\n\nGenerating Your  Playlist To Database...Sabar Ngentot.\n\nGenre:- {genre}"
        )
    ...
    if _check:
        return await CallbackQuery.message.reply_text(
            f"{CallbackQuery.from_user.mention}, Already have in playlist!"
        )
    assis = {
        "videoid": videoid,
        "title": title,
    ...
    return await CallbackQuery.message.reply_text(
        f"Adding {type}'s {genre} Playlist By {CallbackQuery.from_user.mention}"
    )
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"No {genre} Playlist in servers. Try to add playlist.",
            show_alert=True,
        )
    else:
        j = 0
